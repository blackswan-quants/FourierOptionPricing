...
def heston_weighted_loss(
    ...
    # Evaluate Feller Condition: 2*kappa*theta > sigma_v^2
    # If violated, the variance process can reach zero — penalise heavily.
    feller_val = 2 * kappa * theta - sigma_v**2
    penalty = 0.0 if feller_val > 0 else 1e6 * abs(feller_val) + 1000
    ...
    # Liquidity-weighted MSE: weight = 1 / bid_ask_spread (normalised)
    ...
    residuals = model_prices - market_prices
    return float(np.sum(weights * residuals**2)) + penalty
...
def calibrate_heston_model(
    ...
    # workers=1: avoids a macOS multiprocessing deadlock that occurs when the
    # package is loaded via sys.path rather than installed.  On spawn-based
    # systems (macOS, Python 3.8+), worker processes start fresh and cannot
    # inherit a manually-modified sys.path, so they fail to import
    # fourier_options — causing a silent hang.  Single-process execution is
    # reliable; parallelism can be re-enabled after pip-installing the package.
